...
%% set.
-spec merge_disjoint_keys(set(), ?DICT:orddict(),
                          riak_dt_vclock:vclock(), ?DICT:orddict()) -> ?DICT:orddict().
...
-spec clocks_equal(?DICT:orddict(), ?DICT:orddict()) -> boolean().
clocks_equal([], _) ->
    true;
clocks_equal([{Elem, Clock1} | Rest], Entries2) ->
    Clock2 = ?DICT:fetch(Elem, Entries2),
...
-spec remove_elem({ok, riak_dt_vclock:vclock()} | error,
                  member(), {riak_dt_vclock:vclock(), ?DICT:orddict(), deferred()}) ->
                         {ok, {riak_dt_vclock:vclock(), ?DICT:orddict(), deferred()}} |
                         precondition_error().
