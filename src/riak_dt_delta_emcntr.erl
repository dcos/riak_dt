...
%% -------------------------------------------------------------------
%%
%% riak_dt_delta_emcntr: A convergent, replicated, state based PN counter,
...
-type emcntr() :: {riak_dt_vclock:vclock(), [entry()], deferred_deltas()}.
-type entry() :: {Actor :: riak_dt:actor(), {Event :: pos_integer(),
                                             Inc :: pos_integer(),
                                             Dec :: pos_integer()}
                 }.

%% a delta operation should be deferred until it is visible, which
%% means the event dot of the delta is at least contiguous with the
%% emcntr clock
-type deferred_deltas() :: ordsets:ordset(deferred_delta()).
-type deferred_delta() :: entry().
...
-spec new() -> emcntr().
new() ->
    {riak_dt_vclock:fresh(), orddict:new(), ordsets:new()}.
...
parent_clock(Clock, {_, Cntr, Def}) ->
    {Clock, Cntr, Def}.

-spec get_deferred(emcntr()) -> [].
get_deferred(_CRDT) ->
    %% @TODO do emcntr deferred's count as regular deferred
    %% operations?
    [].

%% @doc the current integer value of the counter
-spec value(emcntr()) -> integer().
value({_Clock, PNCnt, _Def}) ->
    %% Don't take unseen delta's into account
...
delta_update(Op, Dot, Cntr0) ->
    ...
    {ok, {riak_dt_vclock:fresh(), %% No clock
          [], %% No entries
          %% Delta as deferred, see merge for details
          [proplists:lookup(Actor, Dots)]
         }
    }.
...
merge(Cnt, Cnt) ->
    Cnt;
merge({ClockA, CntA, DefA}, {ClockB, CntB, DefB}) ->
    Clock = riak_dt_vclock:merge([ClockA, ClockB]),
    {Cnt0, BUnique} = merge_left(ClockB, CntA, CntB),
    Cnt = merge_right(ClockA, BUnique, Cnt0),
    Def = ordsets:union(DefA, DefB),
    strip_deltas(Clock, Cnt, Def).

%% @private any dot in a delta that is at least contiguous with the clock gets
%% promoted up to entries, and dropped for delta list.
    ...
    ordsets:fold(fun({Actor, {Event, Inc, Dec}}=Delta, {C, E, D}) ->
                         ...
                         case (Event - Counter) of
                             N when N < 2 orelse Counter == 0 ->
                                 ...
                                 %% value. Apply @TODO Merge clock
                                 %% (how does this get back up to the
                                 %% MAP!) In fact, it doesn't matter.
                                 {riak_dt_vclock:merge([C, [{Actor, Event}]]),
                                  orddict:update(Actor, fun({Evt, _I, _D}) when Event > Evt ->
                                                                {Event, Inc, Dec};
                                                           (Current) ->
                                                                Current
                                                        end,
                                  ...
                                  D};
                             _->
                                 %% Keep this delta for later
                                 {C, E, ordsets:add_element(Delta, D)}
                         end
                 end,
                 {Clock, Entries, ordsets:new()},
                 Deltas).
